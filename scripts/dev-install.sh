...
echo ""
echo "MCP: dev-install stops the daemon on purpose. In Cursor:"
echo "  Settings → MCP → toggle stringwork OFF then ON (or Reload MCP servers)."
echo "  The proxy will start a fresh daemon on the next connection."
echo "  If it still shows disconnected, check: tail -30 $CONFIG_DIR/mcp-stringwork.log"
